...
-   **新規ユーザー登録**
    -   ユーザーはユーザーIDとパスワード、秘密の質問を入力
-   **ログイン機能**
    -   ユーザーはユーザーIDとパスワードを入力   
-   **ログアウト機能**
    -   ユーザーがログアウトできる機能
-   **パスワード再設定機能**
    -   ユーザーはユーザーIDと秘密の質問により、パスワードの変更が可能
-   **秘密の質問再設定機能**
    -   ユーザーはユーザーIDとパスワードにより、ユーザーIDに紐づいた秘密の質問が変更可能
-   **通知表示機能**
    -   ユーザーは新規ユーザーIDやパスワードの登録、予約情報等をWebサイト内の通知を表示できる
    -   過去の通知は表示されない
-   **座席混雑マップ表示機能**
    -   ユーザーは日時を選択すると、10分ごとの座席の予約状態をマップで表示
-   **新規予約機能**
    -   ユーザーは予約情報を入力
    -   **空席確認機能**
        -   ユーザーは席の空き状況を視覚的に確認できる
        -   ユーザーは席を入力すると仮予約が成立する
-   **QRコード・予約情報表示機能**
    -   ユーザーは予約済みの場合には、QRコードと予約情報を表示
    -   ユーザーは新たな新規予約はできない
-   **予約変更機能**  
    -   ユーザーは予約情報の一部を変更できる
-   **予約キャンセル機能**
    -   ユーザーは予約時刻の5分前まではキャンセルできる

#### チェックイン機能

-   **予約有無選択機能**
    -   ユーザーは「予約している」か「予約していない」かを選択する。
    -   **予約している場合**
        -   ユーザーはチェックインシステムに予約時刻の3分前までにQRコードをかざす
    -   **予約していない場合**
        -   ユーザーはチェックインシステムで新規予約を行う
...
-   **ログアウト機能**
    -   管理者がログアウトできる機能
-   **予約管理機能**
    -   管理者は予約されている情報・座席情報の一覧を確認
    -   管理者はユーザーIDまたは予約番号で予約情報を検索できる
    -   **予約取消機能**
        -   管理者は特定の予約を強制的にキャンセルする機能
    -   **座席混雑マップ表示機能**
        -   管理者は日時を選択すると、10分ごとの座席の予約状態をマップで表示
    -   **予約受付強制停止機能**
        -   管理者はシステム全体または特定時間帯の新規予約受付を一時的に停止する機能
-   **チェックインシステム管理機能**
    -   ユーザーがQRコードが読み込めなかった場合、管理者によりユーザーIDまたは整理番号の手入力によりチェックインを行う機能
-   **システム設定機能**
    -   管理者は予約情報のパラメータを設定可能 
