...
-   **ユーザーログイン**
    -   各ユーザーごとのIDとパスワードを設定
        -   追加に関しては、不正な文字列でないかチェックした上で、自動でデータベースに追加
...
-   **予約情報**
    -   予約に関する詳細データ
    -   予約番号、人数（1～8人）、予約日時、滞在予定時間、メールアドレス、座席指定等
-   **予約番号**
    -   各予約を識別するためのID
...
-   **新規ユーザー登録**
    -   IDとパスワードを入力し、データベースに追加
    -   不正なID・パスワード（長すぎる等）でないことを確認し、自動で追加
-   **ログイン機能**
    -   IDやパスワードを入れてログイン
    -   予約するユーザーを識別可能
-   **通知機能**
...
-   **新規予約機能**
    -   **予約情報入力機能**
        -   利用人数（1～8人）、希望時間（10分ごと）、滞在予定時間、座席指定等を入力
    -   **入力情報確認機能**
