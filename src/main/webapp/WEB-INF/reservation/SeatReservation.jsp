...
<head>
    ...
    <style>
        ...
        .phone-wrapper { overflow-x: hidden; position: relative; width: 393px; height: 852px; background: #FFFFFF; border: 1px solid #FFFFFF; overflow: auto; box-sizing: border-box; }
...
<%-- ========================== ここから下が修正箇所 ========================== --%>
<script>
    const currentTime = "<%= currentTimeJst %>"; //現在時刻の取得
    const form = document.getElementById('reservationForm'); 
    const stepParam = "${step}"; //どの画面か取得
    ...
    function initializeMainScreen() {
        ...
        if (goToConfirmButton) {
            ...
            function updatePeopleCount(newCount) {
                ...
                // 新しいステップ名 'update_main' を設定して、人数情報だけを更新して
                document.getElementById('count').textContent = newCount;
                // メイン画面に留まるようにサーバーに指示する
                document.getElementById('next_step').value = 'update_main';
                form.submit(); // フォームを送信してページを再読み込み
            }
            ...
            goToConfirmButton.onclick = function() {
                // すべての入力が完了したら 'provisional' ステップへ
